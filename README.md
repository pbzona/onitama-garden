...
- **Frame pacing:** the game renders at up to 60 fps while stones, cards or the camera are animating or you're interacting, 30 fps when idle (including the slow menu and victory spins), and 15 fps when the window isn't focused. It never runs uncapped on high-refresh monitors, and browsers pause it completely in background tabs.
...
- **Shadows on demand:** the shadow map only re-renders while stones or cards are moving (or a card is lifting under the cursor).
...
- **`/benchmarks`** (contributed in PR #3 by @raidalt) measures real per-frame GPU time with WebGL timer queries and isolates shadows, bloom, grading and sand. Add `?quality=low` or `?autorun=1`. `vercel.json` rewrites the route to the app.
